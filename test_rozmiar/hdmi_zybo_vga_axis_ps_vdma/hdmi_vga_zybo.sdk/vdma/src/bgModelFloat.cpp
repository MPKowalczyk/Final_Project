...
bgModelFloat::bgModelFloat(unsigned int XX, unsigned int YY)
{
	mf_alpha_1 = ALPHA_1;
	mf_alpha_11 = 1-mf_alpha_1;

	mf_alpha_2 = 1 -mf_alpha_11/15;
	mf_alpha_21 =mf_alpha_11/15;

	mf_weightY =Y_WEIGHT;
	mf_weightCbCr = CBCR_WEIGHT;

	mi_bgThreshold = BG_TH;
	mi_fdThreshold = FD_TH;

	mui_XX = XX;
	mui_YY = YY;

	bgModel   = new float[mui_XX*mui_YY * 3];
...
// -------------------------------------------------------------------------------------------------------------------
void bgModelFloat::init(u8 *image) {
...
//
// -------------------------------------------------------------------------------------------------------------------
void bgModelFloat::segmentation(u8 *image, u8* fgMask, u8* moMask, bbox bb) {
	...
	for (int k = 0; k < mui_YY*mui_XX * 3; k += 3) {
		...
		if ( mf_weightY*std::abs(float(image[k]) - bgModel[k]) + mf_weightCbCr*(std::abs(float(image[k + 1]) - bgModel[k + 1]) + std::abs(float(image[k + 2]) - bgModel[k + 2])) > mi_bgThreshold) {
            ...
            //if (mpu8_fgCounter[binaryImageCnt] > 5)
		...
		if (std::abs(image[k] - prevFrame[binaryImageCnt]) > mi_fdThreshold)
			moMask[binaryImageCnt] = 255;
		else
			moMask[binaryImageCnt] = 0;

        // Save previous frame
		prevFrame[binaryImageCnt] = image[k];
...
//
// ------------------------------------------------------------------------------------------------------------------
 void bgModelFloat::updateAll(u8 *image) {
    for (int k = 0; k < mui_YY*mui_XX * 3; k += 3) {
...
// TODO - optimize
// -------------------------------------------------------------------------------------------------------------------
void bgModelFloat::getBG(u8 *backgroundModel) {
	for (int k = 0; k < mui_YY*mui_XX * 3; k += 1)
		backgroundModel[k] = (u8)(bgModelLT[k]);
}
